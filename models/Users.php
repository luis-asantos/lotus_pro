...
class Users extends model {
	...
	public function add($email, $pass, $group, $id_company) {
		$sql = $this->db->prepare("SELECT COUNT(*) as c FROM users WHERE email = ?");
		...
		if($row['c'] == '0') {
			...
			$sql = $this->db->prepare("INSERT INTO users (email, password, id_group, id_company) VALUES (?,?,?,?)");
			$sql->bindValue(1, $email);
			$sql->bindValue(2, md5($pass));
			$sql->bindValue(3, $group);
			$sql->bindValue(4, $id_company);
			$sql->execute();
